...
var_to_plot <- c("incidD", "incidC")
# Number of test runids
n_runid <- length(unique(sim_stats$runid))
# Whether to group results by runid
by_runid <- n_runid > 3
by_runid = T
...
p_sim <- sim_stats %>%
  filter(var %in% var_to_plot, nnodes == N4plot) %>%
  ggplot(aes(x = time)) +
...
if (by_runid) {
  sim_error_median <- sim_error_median %>% 
    {
      if (grp_by_incid) {
        group_by(., it, var, runid, stat, npi_specs, conf_specs, incid_grp)
      } else {
        group_by(., it, var, runid, stat, npi_specs, conf_specs)
      }
    } %>% 
  ...
  if (grp_by_incid) {
    p_sim_err + facet_grid(npi_specs + conf_specs  + var ~  incid_grp, scales = "free", drop = T)
  } else {
    p_sim_err + facet_wrap(npi_specs + conf_specs ~ var , scales = "free")
  }
...
```{r finalerrorsim, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 10, fig.height = 8, fig.cap="Final statistic (bias or mae) of the simualated trajectories rate. Boxplots show the distribution of values across geoids."}

if (grp_by_incid) {
  # Compute final errors
  final_mean_stat_sims <- sim_errors %>% 
    inner_join(incid_grps) %>% 
    group_by(var, geoid, stat, runid, incid_grp) %>% 
    slice_max(it) %>% 
    summarise(mean_stat = mean(value))
  
  ggplot(final_mean_stat_sims, aes(x = mean_stat, y = runid, color = runid)) +
    geom_boxplot(outlier.size=.8) +
    theme_bw() +
    facet_grid(stat~incid_grp) +
    theme(axis.text.y =  element_blank(),
          legend.position = "bottom")  +
    guides(color=guide_legend(ncol=2)) +
    labs(x = "Mean of statistic over the last iteration")
  
} else {
  # Compute final errors
  final_mean_stat_sims <- sim_errors %>% 
    group_by(var, geoid, stat, runid) %>% 
    slice_max(it) %>% 
    summarise(mean_stat = mean(value))
...
```{r speedsim, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 10, fig.height = 8, fig.cap="Mean number of iterations to readh the  final statistic (bias or mae) of the simualated trajectories rate. Boxplots show the distribution of values across geoids."}

threshold <- 1.1
speed_stats_sims <- sim_errors %>% 
  inner_join(final_mean_stat_sims %>% 
               group_by(var, geoid, stat) %>% 
               summarise(target_stat = min(abs(mean_stat)))) %>% 
  group_by(var, geoid, stat, runid, sim) %>% 
  filter(value < threshold * target_stat) %>% 
  group_by(var, geoid, stat, runid, sim) %>%
  summarise(it = mean(it))

if (grp_by_incid) {
  speed_stats_sims %>% 
    inner_join(incid_grps) %>% 
    ggplot(aes(it, y = runid, color = runid)) +
    geom_boxplot(outlier.size=.8) +
    facet_grid(incid_grp~stat+var) +
    theme_bw() +
    theme(axis.text.y =  element_blank(),
          legend.position = "bottom")  +
    guides(color=guide_legend(ncol=2)) +
    labs(x = "Mean of number of iterations to be within 10% of the final value")
} else {
  speed_stats_sims %>% 
    ggplot(aes(it, y = runid, color = runid)) +
    geom_boxplot(outlier.size=.8) +
...
if (by_runid) { 
  ...
  p_hpar <- hpar %>% 
    ...
    geom_vline(data = hpar_generation  %>%
                 filter((outcome %in% hpar_priors$outcome) & (source %in% hpar$source)),
               aes(xintercept = value), lty = 2, size = .5) +
    geom_density(aes(color = geoid, lty = runid)) +
    guides(color = "none") +
    coord_cartesian(xlim = c(0,1)) +
    theme_bw() +
    theme(legend.position = "bottom")
...
if (grp_by_incid) {
  if (by_runid) {
    p_hpar + facet_grid( npi_specs + conf_specs ~ incid_grp, scales = "free_y", drop = T)
  } else {
...
intercepts_hpar <- tibble(y  = 0, stat = params$stat4plot) 

if (by_runid) {
  hpar_errors_median2 <- hpar_errors_median %>% 
    {
      if (grp_by_incid) {
        group_by(., it, var, runid, stat, npi_specs, conf_specs, incid_grp)
      } else {
  ...
  p_hpar_err <- hpar_errors_median2 %>%  
    filter(stat == stat4plot) %>% 
    ggplot(aes(x = it, y = median, color = runid)) +
    ...
    theme_bw() +
    guides(color=guide_legend(ncol=2)) +
    labs(x = "iteration", y = glue::glue("Mean across geoids of the median {stat4plot} across slots for each goid")) +
    theme(legend.position = "bottom")
  
  if (grp_by_incid) {
    p_hpar_err + facet_grid(npi_specs + conf_specs ~ incid_grp, scales = "free", drop = T)
  } else {
    p_hpar_err + facet_wrap(npi_specs + conf_specs ~ . , scales = "free")
  }
...
```
```{r speedhpar, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 10, fig.height = 7, fig.cap="Final statistic (bias or mae) of the confirmation rate. Boxplots show the distribution of values across geoids."}

if (grp_by_incid) {
  # Compute final errors
  final_mean_stat_hpar <- hpar_errors %>% 
    inner_join(incid_grps) %>% 
    group_by(var, geoid, stat, runid, incid_grp) %>% 
    slice_max(it) %>% 
    summarise(mean_stat = mean(value))
  
  ggplot(final_mean_stat_hpar, aes(x = mean_stat, y = runid, color = runid)) +
    geom_boxplot(outlier.size=.8) +
    theme_bw() +
    facet_grid(stat~incid_grp) +
    theme(axis.text.y =  element_blank(),
...
```{r speed, echo=FALSE, message=FALSE, warning=FALSE, fig.width = 10, fig.height = 7, fig.cap="Number of iterations to be withing 10% of the final statistic (bias or mae) of the confirmation rate. Boxplots show the distribution of values across geoids."}

threshold <- 1.1
speed_stats <- hpar_errors %>% 
  inner_join(final_mean_stat_hpar %>% 
               group_by(var, geoid, stat) %>% 
               summarise(target_stat = min(abs(mean_stat)))) %>% 
  group_by(var, geoid, stat, runid, sim) %>% 
  filter(value < threshold * target_stat) %>% 
