...
if [ -n "$S3_LAST_JOB_OUTPUT" ]; then
	...
	do
		export FILENAME=$(python -c "from SEIR import file_paths; print(file_paths.create_file_name('$COVID_RUN_INDEX','$COVID_PREFIX/$COVID_RUN_INDEX/chimeric/intermediate/%09d.'% $COVID_SLOT_INDEX,$LAST_JOB_BLOCK,'$type','parquet'))")
		aws s3 cp --quiet $S3_RESULTS_PATH/$FILENAME $FILENAME
	done
	for type in "seed"
	do
		export FILENAME=$(python -c "from SEIR import file_paths; print(file_paths.create_file_name('$COVID_RUN_INDEX','$COVID_PREFIX/$COVID_RUN_INDEX/chimeric/intermediate/%09d.'% $COVID_SLOT_INDEX,$LAST_JOB_BLOCK,'$type','csv'))")
		aws s3 cp --quiet $S3_RESULTS_PATH/$FILENAME $FILENAME
	done
	for type in "seed"
	do
		export FILENAME=$(python -c "from SEIR import file_paths; print(file_paths.create_file_name('$COVID_RUN_INDEX','$COVID_PREFIX/$COVID_RUN_INDEX/global/intermediate/%09d.'% $COVID_SLOT_INDEX,$LAST_JOB_BLOCK,'$type','csv'))")
		aws s3 cp --quiet $S3_RESULTS_PATH/$FILENAME $FILENAME
	done
	for type in "hosp" "llik" "spar" "snpi" "hpar" "seir"
	do
		export FILENAME=$(python -c "from SEIR import file_paths; print(file_paths.create_file_name('$COVID_RUN_INDEX','$COVID_PREFIX/$COVID_RUN_INDEX/global/intermediate/%09d.'% $COVID_SLOT_INDEX,$LAST_JOB_BLOCK,'$type','parquet'))")
		aws s3 cp --quiet $S3_RESULTS_PATH/$FILENAME $FILENAME
	done
	ls -ltr model_output
fi
